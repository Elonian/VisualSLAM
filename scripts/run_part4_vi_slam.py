...
from visual_slam.pipeline import run_part4
from visual_slam.utils import describe_gpu


def main() -> None:
    parser = argparse.ArgumentParser(description='Run Project 3 Part 4: full visual-inertial SLAM.')
    ...
    for ds in resolve_datasets(args):
        try:
            out = run_part4(args.data_dir, args.output_root, ds, cfg, track_file=args.track_file)
            print(f'[Part4] Dataset {out.dataset_id} complete -> {out.output_dir / "part4_vi_slam.npz"}')
        except Exception as exc:
            had_error = True
            print(f'[Part4] Dataset {ds} failed: {exc}')
